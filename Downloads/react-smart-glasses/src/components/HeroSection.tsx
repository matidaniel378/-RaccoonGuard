...
import { HERO_SUBHEADLINE } from '../config/landingConfig';
...
export const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  return (
    ...
    >
      {/* Fully responsive layout matching the video on desktop, and clean stack on mobile */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-4 items-center w-full z-10">
        
        {/* LEFT COLUMN: Hero content */}
        <div className="lg:col-span-6 xl:col-span-6 flex flex-col justify-center">
          
          {/* Top pill tags: "Visual glance" and "Hands-free" */}
          <motion.div
          ...
          >
            <span
              className={`px-3 py-1 rounded-full text-xs font-normal border backdrop-blur-sm shadow-xs ${
                isDark
                  ? 'bg-zinc-900/60 border-zinc-800 text-zinc-300'
                  : 'bg-white/50 border-zinc-300/80 text-zinc-700'
              }`}
            >
              Visual glance
            </span>
          ...
          {/* Headline: "Think Smarter. React Faster." / "Evolve Daily." */}
          ...
          >
            {/* Primary CTA: "Start for free ->" button */}
            <button
            ...
            {/* Micro-badges: 2 Overlapping compact avatars */}
          ...
          {/* Subheadline paragraph: Placed at lower left on desktop, anchored below image on mobile */}
          <motion.p
            id="hero-subheadline"
            initial={{ opacity: 0, y: 16 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1], delay: 0.4 }}
            className={`mt-2 sm:mt-4 lg:mt-28 xl:mt-32 text-xs sm:text-sm leading-relaxed max-w-[420px] ${
